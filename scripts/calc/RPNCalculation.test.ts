...
describe("Обратная польская запись - калькулятор, обработка ошибок", () => {
    it(`cosinus 30`, () => {
        expect(() => rpnCalculation.cacl('cosinus 30')).toThrow(Error("Несоответствующие скобки или оператор"));
    });

    it(`(2 + 3) % 2`, () => {
        expect(() => rpnCalculation.cacl('(2 + 3) % 2')).toThrow(Error("Несоответствующие скобки или оператор"));
    });
})